...
ROOT = '/opt/extras.ubuntu.com/indicator-usb'

DATA_FILES = [
    (os.path.join(ROOT, 'bin'), glob.glob('bin/*')),
    (os.path.join(ROOT, 'share', 'indicator-usb'), glob.glob('src/*')),
...
def update_desktop_file_fp():
    ...
    for filedesktopin in glob.glob('*.desktop.in'):
        ...
        desktopfile.readfp(codecs.open(
            filedesktopin, encoding='utf-8', mode='r'))
        if len(lns) > 0:
        ...
        with codecs.open(
                filedesktopin, encoding='utf-8', mode='w') as outputfile:
...
def update_desktop_file():
    ...
    for filedesktopin in glob.glob('*.desktop.in'):
        ...
        if os.path.exists(fileout):
            os.remove(fileout)
        fileout = codecs.open(
            './data/%s' % desktopfilename, encoding='utf-8', mode='w')
        fileout.write('[Desktop Entry]\n')
        #
        desktopfile = ConfigParser.ConfigParser()
        desktopfile.optionxform = str
        desktopfile.readfp(codecs.open(
            './%s.in' % desktopfilename, encoding='utf-8', mode='r'))
        if len(lns) > 0:
            for entry in desktopfile.items('Desktop Entry'):
...
class install_data(_install_data):
    def run(self):
        for lang in os.listdir('build/locale-langpack/'):
            lang_dir = os.path.join(ROOT, 'share', 'locale-langpack', lang,
                                    'LC_MESSAGES')
            lang_file = os.path.join('build', 'locale-langpack', lang,
                                     'LC_MESSAGES',
